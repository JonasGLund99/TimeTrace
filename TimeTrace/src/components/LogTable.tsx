...
function LogTable(props: LogTableProps) {
  const [mappings, setMapping] = useState<Map<string, string>>(
    new Map(props.events.map((event) => [event, ""]))
  );
  ...
  function getMappings(): Map<string, string> {
    return mappings;
  }

  return (
    <div className="relative">
      <div className="flex flex-col w-full h-full gap-2 overflow-auto log-table">
        {props.events.map((event: string, i: number) => {
          return <pre>{event} </pre>;
        })}

        {props.displaysMappings ? (
          <div className="absolute top-0 right-0 flex flex-col gap-2 pl-2 bg-white mapping-container">
            {props.events.map((event: string, i: number) => {
              return (
...
export type LogTableProps = {
  ...
  events: string[];
  getMappings: (mappings: Map<string, string>) => void;
};
